...
// theme setup
function backlink_sentry_theme_setup() {
	...
	// load text domain
	load_theme_textdomain('backlink-sentry', get_template_directory() . '/languages');

	// register Primary menu
    register_nav_menus(array(
        'primary' => __('Primary', 'backlink-sentry')
    ));
}
add_action( 'after_setup_theme', 'backlink_sentry_theme_setup', 10 );

// turn off cleaner gallery if Jetpack gallery functions being used
function backlink_sentry_remove_cleaner_gallery() {

	if( class_exists( 'Jetpack' ) && ( Jetpack::is_module_active( 'carousel' ) || Jetpack::is_module_active( 'tiled-gallery' ) ) ) {
		remove_theme_support( 'cleaner-gallery' );
	}
}
add_action( 'after_setup_theme', 'backlink_sentry_remove_cleaner_gallery', 11 );

// register widget areas
function backlink_sentry_register_widget_areas(){

    /* register after post content widget area */
    hybrid_register_sidebar( array(
        'name'         => __( 'Primary Sidebar', 'backlink-sentry' ),
        'id'           => 'sidebar',
        'description'  => __( 'Widgets in this area will be shown in the sidebar next to the main post content', 'backlink-sentry' )
    ) );
}
add_action('widgets_init','backlink_sentry_register_widget_areas');

/* added to customize the comments. Same as default except -> added use of gravatar images for comment authors */
function backlink_sentry_customize_comments( $comment, $args, $depth ) {
        ...
        <article id="comment-<?php comment_ID(); ?>" class="comment">
            <div class="comment-author">
                ...
                if( $comment->user_id === $post->post_author ) {
                    backlink_sentry_profile_image_output();
                } else {
            ...
            <div class="comment-content">
                <?php if ($comment->comment_approved == '0') : ?>
                    <em><?php _e('Your comment is awaiting moderation.', 'backlink-sentry') ?></em>
                    <br />
                <?php endif; ?>
                <?php comment_text(); ?>
            </div>
            <div class="comment-footer">
                ...
                <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'backlink-sentry' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
...
function backlink_sentry_update_fields($fields) {
    ...
    $fields['author'] =
        '<p class="comment-form-author">
            <label class="screen-reader-text">' . __("Your Name", "backlink-sentry") . '</label>
            <input placeholder="' . __("Your Name", "backlink-sentry") . $label . '" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) .
        '" size="30" ' . $aria_req . ' />
    	</p>';

    $fields['email'] =
        '<p class="comment-form-email">
            <label class="screen-reader-text">' . __("Your Email", "backlink-sentry") . '</label>
            <input placeholder="' . __("Your Email", "backlink-sentry") . $label . '" id="email" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) .
        '" size="30" ' . $aria_req . ' />
    	</p>';

    $fields['url'] =
        '<p class="comment-form-url">
            <label class="screen-reader-text">' . __("Your Website URL", "backlink-sentry") . '</label>
            <input placeholder="' . __("Your URL", "backlink-sentry") . ' (optional)" id="url" name="url" type="url" value="' . esc_attr( $commenter['comment_author_url'] ) .
        '" size="30" />
            </p>';

    return $fields;
}
add_filter('comment_form_default_fields','backlink_sentry_update_fields');

function backlink_sentry_update_comment_field($comment_field) {
	
	$comment_field =
        '<p class="comment-form-comment">
            <label class="screen-reader-text">' . __("Your Comment", "backlink-sentry") . '</label>
            <textarea required placeholder="' . __("Enter Your Comment", "backlink-sentry") . '&#8230;" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea>
        </p>';
	
	return $comment_field;
}
add_filter('comment_form_field_comment','backlink_sentry_update_comment_field');

// remove allowed tags text after comment form
function backlink_sentry_remove_comments_notes_after($defaults){

    $defaults['comment_notes_after']='';
    return $defaults;
}

add_action('comment_form_defaults', 'backlink_sentry_remove_comments_notes_after');

// excerpt handling
function backlink_sentry_excerpt() {
    ...
    if($ismore) {
        the_content( __('Continue Reading', 'backlink-sentry') . "<span class='screen-reader-text'>" . get_the_title() . "</span>");
    }
...
function backlink_sentry_excerpt_read_more_link($output) {
	global $post;
	return $output . "<p><a class='more-link' href='". get_permalink() ."'>" . __('Continue Reading', 'backlink-sentry') . "<span class='screen-reader-text'>" . get_the_title() . "</span></a></p>";
}

add_filter('the_excerpt', 'backlink_sentry_excerpt_read_more_link');

// switch [...] to ellipsis on automatic excerpt
function backlink_sentry_new_excerpt_more( $more ) {
	return '&#8230;';
}
add_filter('excerpt_more', 'backlink_sentry_new_excerpt_more');

// turns of the automatic scrolling to the read more link 
function backlink_sentry_remove_more_link_scroll( $link ) {
	$link = preg_replace( '|#more-[0-9]+|', '', $link );
	return $link;
}
add_filter( 'the_content_more_link', 'backlink_sentry_remove_more_link_scroll' );

// change the length of the excerpts
function backlink_sentry_custom_excerpt_length( $length ) {
    return 20;
}
add_filter( 'excerpt_length', 'backlink_sentry_custom_excerpt_length', 99 );

// Adds navigation through pages in the loop
function backlink_sentry_post_navigation() {
    if ( current_theme_supports( 'loop-pagination' ) ) loop_pagination();
}

// for displaying featured images
function backlink_sentry_featured_image() {
...
function backlink_sentry_add_homepage_title( $title )
{
...
add_filter( 'wp_title', 'backlink_sentry_add_homepage_title' );
...
function backlink_sentry_social_site_list(){
...
function backlink_sentry_social_array(){

	$social_sites = array(
		'twitter' => 'backlink_sentry_twitter_profile',
		'facebook' => 'backlink_sentry_facebook_profile',
		'googleplus' => 'backlink_sentry_googleplus_profile',
		'pinterest' => 'backlink_sentry_pinterest_profile',
		'linkedin' => 'backlink_sentry_linkedin_profile',
		'youtube' => 'backlink_sentry_youtube_profile',
		'vimeo' => 'backlink_sentry_vimeo_profile',
		'tumblr' => 'backlink_sentry_tumblr_profile',
		'instagram' => 'backlink_sentry_instagram_profile',
		'flickr' => 'backlink_sentry_flickr_profile',
		'dribbble' => 'backlink_sentry_dribbble_profile',
		'rss' => 'backlink_sentry_rss_profile',
		'reddit' => 'backlink_sentry_reddit_profile',
		'soundcloud' => 'backlink_sentry_soundcloud_profile',
		'spotify' => 'backlink_sentry_spotify_profile',
		'vine' => 'backlink_sentry_vine_profile',
		'yahoo' => 'backlink_sentry_yahoo_profile',
		'behance' => 'backlink_sentry_behance_profile',
		'codepen' => 'backlink_sentry_codepen_profile',
		'delicious' => 'backlink_sentry_delicious_profile',
		'stumbleupon' => 'backlink_sentry_stumbleupon_profile',
		'deviantart' => 'backlink_sentry_deviantart_profile',
		'digg' => 'backlink_sentry_digg_profile',
		'git' => 'backlink_sentry_git_profile',
		'hacker-news' => 'backlink_sentry_hacker-news_profile',
		'steam' => 'backlink_sentry_steam_profile',
		'vk' => 'backlink_sentry_vk_profile',
		'weibo' => 'backlink_sentry_weibo_profile',
		'tencent-weibo' => 'backlink_sentry_tencent_weibo_profile',
		'email' => 'backlink_sentry_email_profile'
	);
	return $social_sites;
}

// retrieves the attachment ID from the file URL
function backlink_sentry_get_image_id($url) {
...
function backlink_sentry_profile_image_output(){

    // use User's profile image, else default to their Gravatar
    if(get_the_author_meta('backlink_sentry_user_profile_image')){

        // get the id based on the image's URL
        $image_id = backlink_sentry_get_image_id(get_the_author_meta('backlink_sentry_user_profile_image'));
...
function backlink_sentry_wp_backwards_compatibility() {
...
add_action('init', 'backlink_sentry_wp_backwards_compatibility');
 ...
 */
function backlink_sentry_set_date_format() {

	// if the date format has never been set by backlink_sentry, set it
	if( get_option('backlink_sentry_date_format_origin') != 'updated' ) {
		update_option('date_format', 'F j, Y');

		// add option so never updates date format again. Allows users to change format.
		add_option('backlink_sentry_date_format_origin', 'updated');
	}
}
add_action( 'init', 'backlink_sentry_set_date_format' );

function backlink_sentry_output_svg($svg) {
